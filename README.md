...
PocketLedger 是一个基于 Kotlin 和 Jetpack Compose 的 Android 本地记账应用，安装到手机后的显示名称为「口袋账本」。除了日常记账和 AA 分账，应用还提供一套完整的旅行助手，用于管理行程、预算、同行成员、旅行支出、AA 结算、打包清单和行程提醒。当前版本为单机应用，数据保存在应用私有目录中，不依赖后端服务。
